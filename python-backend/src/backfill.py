...
def fetch_historical_range(days: int) -> pd.DataFrame:
    """Fetch historical AQI and weather data from Open-Meteo."""
    ...
    # Smaller chunks reduce API timeouts
    chunk_days = 14
    cursor = start
    ...
    if df_pollution.empty:
        raise RuntimeError(
            "No historical air quality data returned from Open-Meteo."
        )

    df_pollution["datetime"] = pd.to_datetime(
        df_pollution["datetime"], utc=True
    )
    df_weather["datetime"] = pd.to_datetime(
        df_weather["datetime"], utc=True
    )
...
def run(days: int = 730, triggered_by: str = "manual"):
    ...
    logger.info(
        "=== Backfilling %d days of historical AQI data for %s ===",
        days,
        config.CITY_NAME,
    )
    ...
    try:
        ...
        featurized = full_feature_pipeline(
            raw,
            target_col="aqi",
        )

        lag_columns = [
            c for c in featurized.columns
            if "lag" in c
        ]
        ...
        logger.info(
            "Backfill complete: %d feature rows written.",
            len(featurized),
        )
        ...
        log_run_end(
            run_record,
            status="success",
            records_processed=len(featurized),
            extra_logs=[
                f"Fetched {len(raw)} raw hourly records from Open-Meteo",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=730,
        help="How many past days to backfill (default: 730)",
    )
